...
@Component({
  selector: 'app-dashboard-defaul',
  templateUrl: './dashboard-defaul.component.html',
  styleUrls: ['./dashboard-defaul.component.css'],
  animations: [
...
export class DashboardDefaulComponent implements OnInit {
  ...
  editarProjeto(projeto) {
      ...
      this.blockUI.start();
        this.projetosService.atualizar(projeto).pipe(
            finalize(() => this.blockUI.stop())
  ...
  editarOs(os) {
      ...
      this.blockUI.start();
          this.osService.atualizar(os).pipe(
              finalize(() => this.blockUI.stop())
